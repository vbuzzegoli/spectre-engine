...
# Inspired by: https://github.com/jamf/icns-Creator/blob/master/icns_creator.sh

cprint=./scripts/helpers/cprint.sh

src_image="$1"
if [ -z "$1" ]; then
    $cprint error "No source image was passed for conversion"
    exit 1
fi
...
if [ "${src_image:(-3)}" != "png" ]; then
    ...
    if [ $? -ne 0 ]; then
        $cprint error echo "The source image could not be converted to PNG format."
        exit 1
    fi
...
if [ -e "$iconset_path" ]; then
    ...
    if [ $? -ne 0 ]; then
        $cprint error "There is a pre-existing file/dir $iconset_path that could not be deleted"
        exit 1
    fi
fi
...
for a in ${icon_file_list[@]}; do
    ...
    /usr/bin/sips -z $icon_size $icon_size "$icon" | grep "skip log"
    counter=$(($counter + 1))
done
...
if [ $? -ne 0 ]; then
    $cprint error "There was an error creating the .icns file"
    exit 1
fi
